...
<!-- 여기서 파라미터값을 입력하면 param값은 JoinHandler에 processSubmit 메소드의 getParam..로 넘어간다  -->
	...
	<tr>
		<td>닉네임(id)</td>
		<td colspan="2"><input type="text" name="id" value="${param.id }" placeholder="닉네임" maxlength="10">
	...
	<tr>
		<td>비밀번호</td>
		<td colspan="2"><input type="password" name="pw" value="${param.pw }" placeholder="비밀번호 4자리이상, 12자리이하" minlength="4" maxlength="12">
		<c:if test="${errors.pw }">비밀번호를 입력하세요</c:if></td>
	</tr>

	<tr>
		<td>비밀번호 확인</td>
		<td colspan="2"><input type="password" name="confirmPw" value="${param.confirmPw }" placeholder="비밀번호 확인" minlength="4" maxlength="12">
